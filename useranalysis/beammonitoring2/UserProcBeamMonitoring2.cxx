...
//	DetEventStation* st_F5t = (DetEventStation*)(v_input->getEventElement("Beam_detector_tF5",1));
	...
	//daria, getting time from advbm
	DetEventStation* st_MWPCtime = (DetEventStation*)(v_input->getEventElement(fParBD->GetStationMWPCtimename().Data(),1));
	if(!st_MWPCtime) {
		cout << " station " << fParBD->GetStationMWPCtimename() << " was found in event " << fEventCounter << endl;
	}

	MWPCprojection(st_MWPCX1, st_MWPCY1, st_MWPCX2, st_MWPCY2, curTrigger);

	//daria trying to get times
	MWPCtime(st_MWPCtime, curTrigger);

	return;

}

void UserProcBeamMonitoring2::MWPCtime(DetEventStation* mwpc_time, Int_t curTrigger){

	
	TClonesArray* tmwpc = NULL;
	Int_t nentries;
	Float_t temp[4];
	Float_t temp2[4];
...
void UserProcBeamMonitoring2::MWPCprojection(DetEventStation* mwpcPlaneX1,
	...
	//daria
	Float_t xplanew, yplanew, xplanec, yplanec /*, xtw_dar, ytw_dar, */;
...
//	return;
	...
	if (v_MWPC[0] && v_MWPC[1] && v_MWPC[2] && v_MWPC[3] && curTrigger==fParBD->fTriggerCondition) {
		...
		if (wireMult[0]==1 && wireMult[1]==1 && wireMult[2]==1 && wireMult[3]==1) {
			...
			//daria
			//xtw_dar = fParBD->fMWPCz1*(x2p - x1p)/(fParBD->fMWPCz2-fParBD->fMWPCz1) - x1p;
			//ytw_dar = fParBD->fMWPCz1*(y2p - y1p)/(fParBD->fMWPCz2-fParBD->fMWPCz1) - y1p;
			xplanew = x1p - (x2p -x1p)*(fParBD->fMWPCz1 - fParBD->fDistFromTarget)/(fParBD->fMWPCz2-fParBD->fMWPCz1);
			yplanew = y1p - (y2p -y1p)*(fParBD->fMWPCz1 - fParBD->fDistFromTarget)/(fParBD->fMWPCz2-fParBD->fMWPCz1);
			...
			//dariafMWPCProfileW
			//fHistoMan->fTargProjDaria->Fill(xtw_dar, ytw_dar);
			fHistoMan->fPlaneProjW->Fill(xplanew,yplanew);
		...
		if (clusterMult[0]==1 && clusterMult[1]==1 && clusterMult[2]==1 && clusterMult[3]==1) {
			...
			xplanec = x1c - (x2c -x1c)*(fParBD->fMWPCz1 - fParBD->fDistFromTarget)/(fParBD->fMWPCz2-fParBD->fMWPCz1);
			yplanec = y1c - (y2c -y1c)*(fParBD->fMWPCz1 - fParBD->fDistFromTarget)/(fParBD->fMWPCz2-fParBD->fMWPCz1);
//			cout <<
			...
			fHistoMan->fPlaneProjC->Fill(xplanec,yplanec);
